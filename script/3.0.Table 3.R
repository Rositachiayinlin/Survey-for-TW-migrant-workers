...
## ---- 7) Main results: Group effect only ----
...
addStyle(
  ...
  header_style,
  rows = 1,
  cols = 1:ncol(table_main),
  gridExpand = TRUE
)

addStyle(
  wb, "Full regression table",
  header_style,
  rows = 1,
  cols = 1:ncol(table_full),
  gridExpand = TRUE
)
...
message("Table 3 exported successfully: ", file.path(getwd(), output_file))