############################################
## Appendix p5: Fatigue sensitivity
## Data file: data/matchedsample(1302).csv
## Output file: 3.2.Appendix p5 Fatigue sensitivity.xlsx
############################################
...
## ---- 1) Read data ----
file <- "data/matchedsample(1302).csv"
...
## ---- 7) Normality check ----
g1 <- dat_norm %>% filter(Group == 1) %>% pull(Tiredsum)
g0 <- dat_norm %>% filter(Group == 0) %>% pull(Tiredsum)
...
desc_table <- bind_rows(
  cbind(Group = "Taiwanese migrant workers in Vietnam", desc(g1)),
  cbind(Group = "Non-migrant workers in Taiwan", desc(g0))
) %>%
  mutate(
...
## ---- 8) Create normality plots ----
plot_file <- "3.2.Appendix p5 Fatigue sensitivity_plots.png"
...
hist(
  g1,
  main = "Histogram: Tiredsum (Group 1)",
  xlab = "Tiredsum",
  ylab = "Frequency"
)
...
qqnorm(
  g1,
  main = "Q-Q Plot: Tiredsum (Group 1)",
...
hist(
  g0,
  main = "Histogram: Tiredsum (Group 0)",
  xlab = "Tiredsum",
  ylab = "Frequency"
)
...
qqnorm(
  g0,
  main = "Q-Q Plot: Tiredsum (Group 0)",
  ylab = "Sample Quantiles",
  xlab = "Theoretical Quantiles"
)
qqline(g0)
...
## ---- 9) Export only one Excel file ----
output_file <- "3.2.Appendix p5 Fatigue sensitivity.xlsx"
...
message("Fatigue sensitivity results exported successfully: ", file.path(getwd(), output_file))